...
void __vfs_rootset (node_t *root)
{
    if (!_fs_root)
        _fs_root = root;
}
...
int vfs_open (node_t *parent, node_t **node, const char *path, u64 args)
{
    ...
    int res = _vfs_open (parent, node, (char *)path, args);
    if (res < 0)
        DEBUGK(K_FS, "failed to open %s (%#x) - stat : %d\n", path, args, res);
...
int vfs_read (node_t *this, void *buffer, size_t siz, size_t offset)
{
    ...
    if (res < 0)
        DEBUGK(K_FS, "failed to read %s - stat : %d\n", this->name, res);
...
int vfs_write (node_t *this, void *buffer, size_t siz, size_t offset)
{
    ...
    if (res < 0)
        DEBUGK(K_FS, "failed to write %s - stat : %d\n", this->name, res);
...
int vfs_close (node_t *this)
{
    ...
    if (res < 0)
        DEBUGK(K_FS, "failed to close %s - stat : %d\n", this->name, res);
...
int vfs_remove (node_t *this)
{
    ...
    if (res < 0)
        DEBUGK(K_FS, "failed to remove %s - stat : %d\n", this->name, res);
    return res;
}
...
int vfs_truncate (node_t *this, size_t offset)
{
    ...
    if (res < 0)
        DEBUGK(K_FS, "failed to truncate %s - stat : %d\n", this->name, res);
    return res;
}
...
int vfs_readdir (node_t *this)
{
    ...
    if (res < 0)
        DEBUGK(K_FS, "read directory failed - stat : %d\n", res);
