        ...
        )
{
    ...
    if (EFI_ERROR (Status))
    {
        DEBUG ((DEBUG_ERROR ,"[FAIL] Unable to open file : %s - Status : %r\n",Path,Status));
        return Status;
    }
    ...
    if (EFI_ERROR (Status))
    {
        DEBUG ((DEBUG_ERROR ,"[FAIL] Unable to read the font file - Status : %r\n",Status));
        return Status;
    }

    Font->AscOff = ConfigGetNumUint64 ("FontAscOffset",0);
    Font->Width  = ConfigGetNumUint64 ("FontWidth",0);
    Font->Height = ConfigGetNumUint64 ("FontHeight",0);
